...
create procedure Doctores as select * from NumeroEm
Exec NumeroEm
Create procedure Enfermera as select * from  NumeroEm
Exec NumeroEm
Create procedure DatPacientes as select * from IdPac
Exec IdPac
Create procedure Antecedentes as select * from IdPac
Exec IdPac
Create procedure Medicamentos as select * from IdMedicamento
Exec IdMedicamento

create function DOC(@NumeroEm int) returns table as return (select * from Doctores where NumeroEm = @NumeroEm)
Select * from DOC(2)

Create function DatosPac (@Nombre varchar(20), @Edad nchar(2), @FechaNac nvarchar(30)) return nvarchar(50) as begin return (Select @Nombre+ ' ' +@Edad+ ' ' +@FechaNac) End
select DatosPac (Nombre, Edad, FechaNac) as detaitl from DatPacientes

Create trigger Imedi on Medicamentos instead of delete as begin print ( 'No tiene acceso para realizar esto') end
delete from Medicamentos Where Imedi= 5

Create Trigger IRec on Recete after delete as begin print ('Se ha eliminado'); end
delete from Receta where IRec = 6